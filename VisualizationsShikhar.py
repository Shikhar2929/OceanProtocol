    ...
    def __init__(self, price_df, hours = 2, frequency = 1):
        ...
        for index, row in self.curr_df.iterrows():
            if len(self.curr_df['close']) < self.adjust + index:
                ...
                #l.append([])
                continue
            if (index % self.window == 0 and index > 5000):
                self.curr_df['Y'].iloc[index] = self.curr_df['close'].iloc[index + self.adjust]
                if (self.curr_df['close'].iloc[index] + 20 < self.curr_df['Y'].iloc[index] or 
                    self.curr_df['close'].iloc[index] - 20 > self.curr_df['Y'].iloc[index]):
        ...
        self.finalDataFrame.to_csv("ETH_DATA/10_minute_ML_2_hour_NewOnly.csv")
